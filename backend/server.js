...
// Conditional requires for optional services
let twilio = null;
...
function withJoins(task) {
  ...
  return {
    ...
    assignee_language: assignee?.language || 'en',
...
app.get('/api/tasks', (req, res) => {
  const all = tasksCol.chain().data().map(withJoins).sort((a, b) => b.created_at.localeCompare(a.created_at));
  res.json(all);
});
...
app.get('/api/tasks/:id', (req, res) => {
  ...
  const logs = logsCol.find({ task_id: req.params.id })
    .map(l => {
      const u = l.changed_by ? cleanDoc(usersCol.findOne({ id: l.changed_by })) : null;
      return { ...cleanDoc(l), changed_by_name: u?.name || 'System' };
    })
...
app.post('/api/tasks', async (req, res) => {
  ...
  const id = uuidv4();
  const task = tasksCol.insert({
    id, title, description: description || '',
    status: 'Created', priority: priority || 'Medium',
    assignee_id: assignee_id || null, created_by,
  ...
  if (result.assignee_phone) {
    ...
    translateText(msgEN, result.assignee_language).then(msg =>
      sendWhatsApp(result.assignee_phone, msg).catch(console.error));
  }
...
app.delete('/api/tasks/:id', (req, res) => {
  const task = tasksCol.findOne({ id: req.params.id });
  if (task) {
    logsCol.removeWhere({ task_id: req.params.id });
    tasksCol.remove(task);
    db.saveDatabase();
  }
  res.json({ ok: true });
});
...
app.post('/api/tasks/:id/notify', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, translated, simulated: result.simulated || false });
  } catch (err) {
...
app.get('/api/stats', (req, res) => {
  ...
  res.json({
    total: all.length,
    by_status: Object.entries(by_status).map(([status, count]) => ({ status, count })),
    overdue
  });
});
